...
def store_a_value():
    ...
    if tag:
        if tag == 'dbpass':
            return return_error('Can not use this tag.')
        else:
            # --------------------
            if getpassword:
    ...
    return return_error('Specify a tag')
...
def get_data():
    ...
    return return_error('There is no password set.')
...
def delete_entry():
    ...
    if tag:
        if tag == 'dbpass':
            return return_error('Can not delete password record.')
        else:
            # --------------------
            if getpassword:
                password = request.form['pass']
...
def set_key():
    ...
    return return_error('Specify a new password')
...
def not_found(e):
    return jsonify(action="ERROR",result="The requested URL was not found on the ViewTubeDB instance!"), 404
